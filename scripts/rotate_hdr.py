...
输出: <hdr名>_rotate.bin (翻转后副本, 存于 rotate_dir; 默认 = hdr_dir)

垂直翻转 = 翻转行序 (raw[::-1, :]); 仅上下颠倒需此 (非 180° 旋转, 否则会引入左右镜像)。
无损操作 (仅重排像素, 不经 PQ 解码/编码)。
路径参数从 config.json (task0 节) 读取; task0.hdr_dir 默认 task1.hdr_dir; task0.rotate_dir 默认 task0.hdr_dir。
用法:
...
def main():
    ...
    ap.add_argument('--hdr-dir', default=None, help='覆盖 config.task0.hdr_dir (原始 HDR bin 输入目录)')
    ap.add_argument('--rotate-dir', default=None, help='覆盖 config.task0.rotate_dir (_rotate.bin 输出目录, 默认 hdr_dir)')
    args = ap.parse_args()

    cfg = load_config(args.config)
    common.set_dims(cfg['common']['height'], cfg['common']['width'])

    # 输入目录 (原始 bin): task0.hdr_dir, 默认 task1.hdr_dir
    ...
    if not os.path.isdir(hdr_dir):
        raise SystemExit(f"输入目录不存在: {hdr_dir}")

    # 输出目录 (_rotate.bin): task0.rotate_dir, 默认 hdr_dir
    rotate_dir_cfg = task0_cfg.get('rotate_dir', '')
    if args.rotate_dir:
        rotate_dir = resolve_path(args.rotate_dir)
    elif path_filled(rotate_dir_cfg):
        rotate_dir = resolve_path(rotate_dir_cfg)
    else:
        rotate_dir = hdr_dir
    ...
    print(f"HDR bins: {len(files)}, hdr_dir={hdr_dir}, rotate_dir={rotate_dir}")
    n = 0
    for f in files:
        name = os.path.splitext(os.path.basename(f))[0]
        out = os.path.join(rotate_dir, name + '_rotate.bin')
