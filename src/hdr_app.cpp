...
void HDRApp::computeLocalAvgI() {
    ...
    int by0 = (int)(loY * bgH);
    int by1 = (int)(hiY * bgH);
    if (bx0 >= bx1) { bx0 = 0; bx1 = bgW; }  // fallback: use full width
    if (by0 >= by1) { by0 = 0; by1 = bgH; }  // fallback: use full height
    if (bx1 > bgW) bx1 = bgW;
    if (by1 > bgH) by1 = bgH;
    ...
    for (int y = by0; y < by1; ++y) {
        for (int x = bx0; x < bx1; ++x) {
            ...
            // RGB → LMS (standard BT.2100-2, integer / 4096)
            float L = 0.41210938f*r2 + 0.52392578f*g2 + 0.06396484f*b2;
            float M = 0.16674805f*r2 + 0.72045898f*g2 + 0.11279297f*b2;
